#!/usr/bin/env bash
#SBATCH --job-name=iws-train-stage2
...
# Override path with:
#   sbatch --export=ALL,TRAIN_CONFIG_YAML=/path/to/configurations/isambard_train_stage2.yaml slurm/iws_train_stage2_slurm.sh
# NOTE: default uses REPO_DIR because sbatch runs scripts from Slurm spool paths.
TRAIN_CONFIG_YAML="${TRAIN_CONFIG_YAML:-${REPO_DIR}/configurations/isambard_train_stage2.yaml}"
