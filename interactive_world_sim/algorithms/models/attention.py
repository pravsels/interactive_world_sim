...
def once(fn: Callable) -> Callable:
    ...
    @wraps(fn)
    def inner(x: Any) -> Optional[object]:
        nonlocal called
        if called:
            return None
        called = True
...
print_once: Callable = once(print)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        gpu_name = device_properties.name.upper()
        is_a100 = (
            "A100" in gpu_name
    ...
    def forward(
        self, hidden_states: torch.Tensor, is_causal: bool = False
    ) -> torch.Tensor:
        """Attention mechanism"""
...
MultiViewSpatialAttentionBlock = get_einops_wrapped_module(
    AttentionBlock, "b v c t h w", "(b t) (v h w) c"
)

MultiViewTemporalAttentionBlock = get_einops_wrapped_module(
    _TemporalAttentionBlock, "b v c t h w", "(b v h w) t c"
)

SpatialAttentionPadBlock = get_einops_wrapped_module(
    AttentionBlock, "b c h w", "(b 1) (h w) c"
)

TemporalAttentionPadBlock = get_einops_wrapped_module(
    _TemporalAttentionBlock, "b c h w", "(b h w) 1 c"
)
